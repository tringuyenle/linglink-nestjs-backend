...
export class PostsService {
  ...
  async getPostWithReactById(userId: string, postId: string) {
    ...
    if (post) {
      ...
      const checkReactionStatus = userId
        ? await this.reactionsService.checkPostReactionStatus(userId, post._id.toString())
        : null
  ...
  async getAllPostsByPagev2(
    ...
    userId: string,
    pageSize: number
  ): Promise<{ data: Post; like: boolean; dislike: boolean; numlikes: number; numdislikes: number }[]> {
    const query = {}
