...
export class ReactionsService {
  ...
  async getReactionByPostId(
    postId: string,
  ): Promise<{ likeUsers: Reaction[]; dislikeUsers: Reaction[] }> {
    const userReaction = await this.reactionModel
      .find({ post: postId })
      .populate({ path: 'user', select: '-hashedPassword' })
      .exec();
    const likeUsers = userReaction.filter(
      (user) => user.reactionType === ReactionType.LIKE,
    );
    const dislikeUsers = userReaction.filter(
      (user) => user.reactionType === ReactionType.DISLIKE,
    );
    return { likeUsers: likeUsers, dislikeUsers: dislikeUsers };
  }

  async getReactionByCommentId(
    commentId: string,
  ): Promise<{ likeUsers: Reaction[]; dislikeUsers: Reaction[] }> {
    const userReaction = await this.reactionModel
