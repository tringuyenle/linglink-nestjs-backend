...
export class MessageService {
    ...
    async getMessage(user: User, chatRoomId: string){
        try {
            ...
            const messages = await this.messageModel.find({ chatRoomId: chatRoom.chatRoomId }, { _id: 0, createdAt: 0, updatedAt: 0, __v: 0 }).populate('from', '-hashedPassword -role -createdAt -updatedAt -__v').exec();
            return messages;
        } catch (err) { 
            throw new HttpException('Failed to get message', HttpStatus.INTERNAL_SERVER_ERROR);
        }
    }
