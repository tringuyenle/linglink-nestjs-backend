...
export class AuthService {
  constructor(
    ...
    @InjectModel('ReserPasswordToken')
    private readonly reserPasswordTokenModel: Model<ReserPasswordToken>,
  ) {}
  ...
  async requestPasswordReset(mail: { email: string }) {
    ...
    const token = await this.reserPasswordTokenModel.findOne({
      user_id: user._id,
    });
    ...
    const templateFile = path.join(
      __dirname,
      '../../../utils/email/template/requestResetPassword.pug',
    );
  ...
  async resetPassword(userId: string, token: string, password: string) {
    ...
    const passwordResetToken = await this.reserPasswordTokenModel.findOne({
      user_id: user_id,
    });
